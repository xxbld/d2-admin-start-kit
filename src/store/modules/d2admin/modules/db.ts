...
export default class D2Db extends VuexModule {
   ...
   */
  @Action
  databasePage({ basis = 'fullPath', user = false } = {}) {
    return getDatabase({
   ...
   */
  @Action
  databasePageClear({ basis = 'fullPath', user = false } = {}) {
    return getDatabase({
      path: `$page.${router.app.$route[basis]}`,
      user,
      validator: () => false,
   ...
   */
  @Action
  pageSet({ instance, basis = 'fullPath', user = false }) {
    return getDatabase({
   ...
   */
  @Action
  pageGet({ instance, basis = 'fullPath', user = false }) {
    return dbGet({
      path: `$page.${router.app.$route[basis]}.$data`,
      user,
      defaultValue: cloneDeep(instance.$data)
   ...
   */
  @Action
  pageClear({ basis = 'fullPath', user = false }) {
    return getDatabase({
      path: `$page.${router.app.$route[basis]}.$data`,
      user,
      validator: () => false,
